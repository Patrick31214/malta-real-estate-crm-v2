...
const OwnerDetail = ({ owner, onEdit, onClose, canEdit, canDelete, onDelete, onViewProperty }) => {
  ...
  return (
    <div style={{ padding: 'var(--space-6)', maxWidth: '950px', margin: '0 auto' }}>
      ...
      {owner.Properties && owner.Properties.length > 0 && (
        <div className="glass" style={{ padding: 'var(--space-5)', borderRadius: 'var(--radius-md)', marginBottom: 'var(--space-4)' }}>
          ...
          {owner.Properties.map(p => (
            <div
              key={p.id}
              style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--space-2) 0', borderBottom: '1px solid var(--color-border-light)', fontSize: 'var(--text-sm)', cursor: loadingProperty ? 'wait' : 'pointer' }}
              onClick={() => !loadingProperty && handlePropertyClick(p)}
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '2px', flexShrink: 0, marginLeft: 'var(--space-3)' }}>
                ...
                <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-primary)' }}>🔍 View Details</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Full-screen property overlay */}
      {selectedProperty && (
        <div
          role="dialog"
          aria-modal="true"
          aria-label={`Property details: ${selectedProperty.title || selectedProperty.id}`}
          onClick={(e) => { if (e.target === e.currentTarget) setSelectedProperty(null); }}
          onKeyDown={(e) => { if (e.key === 'Escape') setSelectedProperty(null); }}
          tabIndex={-1}
          style={{
            position: 'fixed',
        ...
        >
          <div style={{
            ...
            <button
              ...
              aria-label="Close property details overlay"
              style={{
                position: 'sticky',
                top: 'var(--space-3)',
                float: 'right',
